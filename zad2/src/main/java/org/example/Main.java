...
public class Main {
    public static void main(String[] args) {
        ...
        Map<Character, Runnable> availableOperations = new HashMap<>();
        List<String> operationsInput = List.of("x = x + y", "y = y + 2z", "x = 3x + z", "z = y - z");

        for (int i = 0; i < operationsInput.size(); i++) {
            availableOperations.put(A.get(i), OperationGenerator.createOperation(operationsInput.get(i), variables));
        }
